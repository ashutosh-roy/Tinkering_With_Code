 ...
 *
 * <p>We'll use combination formula {@code nCr} to solve this. Let's break down the problem into 3
 * parts.
 *
 * <ul>
 *   <li>Step 1. Get each element in Pascal's triangle using {@code nCr} where {@code n} is the
 *       rowth index and {@code r} is the columnth index. <br>
 *       For e.g., For row = 5 col = 3, the element is 6 (from {@code 5C3}) <br>
 *   <li>Step 2. Let's calculate all the elements of a Pascal's triangle for a given row.
 *   <li>Step 3. Combine all those row wise Pascal's triangle elements into a {@code
 *       List<List<Integer>>}. <br>
 *       <h3>Time complexity : </h3>
 *       <p>{@code O(n) + O(n-r) + O(r)}
 * </ul>
 *
 * <br>
 * <br>
 ...
 *
 * <ul>
 *   <li>For e.g., 5C3 is {@code 5! / (3! X 2!)} where {@code (5X4) X 3! / (3! X (2X1))} <br>
 *       So 3! is repeated in numerator and denominator
 *   <li>To avoid the repeated 3! in above e.g., we can do this {@code for(i=1;i<col;i++) { result =
 *       result * (row - i)/i; which for 5C3, result = (5/1) * (4/2) = 10 }}
 * </ul>
 *
 * <br>
 *
 * <h3>Time complexity : </h3>
 *
 * <p>{@code O(n*r*r) ~ O(n3), where n = number of rows, and r = column index.} <br>
 *
 * <h3>Space complexity : </h3>
 *
 * <p>In this case, we are only using space to store the answer. That is why space complexity can be
 * still considered as {@code O(1)} <br>
 *
 * <h3>References:</h3>
 *
 * <a href = "https://www.youtube.com/watch?v=bR7mQgwQ_o8&t=1210s">Video explanation</a><br>
 * <a href =
 * "https://takeuforward.org/data-structure/program-to-generate-pascals-triangle/#brute-force-approach">Reference
 * Code</a>
 */
